...
void setup() {
  Serial.begin(9600);
  Serial.println("Program begins: allocating space");
   /* Key vector */
  uint8_t key_vectors[]
  ...
  uint8_t pText[32]; 
  char pTextstr[] = {"This is a secret message!"};
  for(volatile int i = 0; i < strlen(pTextstr); i++)
  {
    pText[i] = (uint8_t) pTextstr[i];
  }
  ...
  Serial.println("AES initialization: ");
 ...
 Serial.println("AES key generated successfully!...");
  ...
  unsigned char c;
  for(volatile int i = 0; i < strlen(pTextstr); i++, c = i + 'a')
  {
      cTextstr[i] = cText[i];
      pText1str[i] = pText1[i];
  }
  cTextstr[c - 'a'] = '\0';
  pText1str[c - 'a'] = '\0';
  ...
  Serial.println(pTextstr);
  // Print encrypted message
  Serial.println("The encrypted message is:");
  Serial.println(cTextstr);
  // Print decrypted message
  Serial.println("Decrypted message using AES-CBC mode :");
  Serial.println(pText1str);
